...
class DualCoop(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        layers_img = []
        for i in range(len(sizes) - 2):
            layers_img.append(nn.Linear(sizes[i], sizes[i + 1], bias=False))
            layers_img.append(nn.BatchNorm1d(197))
            layers_img.append(nn.ReLU(inplace=True))
        layers_img.append(nn.Linear(sizes[-2], sizes[-1], bias=False))
        self.text_projector = nn.Sequential(*layers_text)
    ...
    def forward(self, image, cls_id=None):
        ...
        img_feat = self.image_projector(img_feat_interm).permute(0,2,1)
        image_features = img_feat[:,:512,:]
